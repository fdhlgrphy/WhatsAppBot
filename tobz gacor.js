...
${pesanafk}`)}`)
        ...
        case prefix+'ttp2':
            ...
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#ttp2 [ Teks ]*, contoh *#ttp2 ZXCBOT*`, id)
        ...
        case prefix+'ttg':
            ...
            break
			 case prefix+'nuliskiri':
                    ...
                    break
                    case prefix+'hilih': 
					if (args.length < 1) return tobz.eply('𝗸𝗮𝘀𝗶𝗵 𝘁𝗲𝗸𝘀 𝗹𝗮𝗵!!!')
					anu = await get.get(`https://mhankbarbars.herokuapp.com/api/hilih?teks=${body.slice(7)}`, {method: 'get'})
					tobz.reply(anu.result)
					break
...
Total Pengguna yang telah terdaftar ${pendaftar.length}
            ...
            case prefix+'daftarulang':
                    if (!isOwner) return tobz.reply(from, 'Command ini hanya dapat digunakan oleh admin ZXCBOT', id)  
...
Total Pengguna yang telah terdaftar ${pendaftar.length}
			...
			ZXCBOT BY JUWENAJAA	`)
				...
				tobz.sendText(from, `Total Pengguna yang telah terdaftar:\n\n*[ ${pendaftar.length} USER ]*`)
				break
...
↬	${texxt}  `
		...
		case prefix+'brppersen':
            ...
            if (!ohhh) tobz.reply(from, '⚠️ Format salah! Ketik *!menu* untuk penggunaan.')
...
➸ *Wilayah:* +${region_name}`
	...
	case prefix+'howgay':
        ...
        case prefix+'tagall': // FOR GROUP ADMINS
            ...
            let hehe = `*PESAN DARI @${sender.id.replace('@c.us','')}*\n─────────────────────\n\n${body.slice(7)}\n\n─────────────────────\n`
...
const pen = no.replace(' ','')
        ...
        case prefix+'ping':
            ...
            case prefix+'ping2':
                if(isReg(obj)) return
                if(cekumur(cekage)) return
                const loadedMsgg = await tobz.getAmountOfLoadedMessages()
                const chatIdss = await tobz.getAllChatIds()
                const groupss = await tobz.getAllGroups()
                const batteryLevel = await tobz.getBatteryLevel()
                const charged = await tobz.getIsPlugged();
                const timestamps = speed();
                const latensis = speed() - timestamps
                tobz.sendText(from, `Penggunaan RAM: ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB\nCPU: ${os.cpus().length}\n\nStatus :\n- ${loadedMsgg} Loaded Messages\n- ${groups.length} Group Chats\n- ${chatIdss.length - groups.length} Personal Chats\n- ${chatIds.length} Total Chats\n\nSpeed: ${latensis.toFixed(4)} Second Status bot : ${'%state'.replace('%state', state.status)} Bot Device Battery Info*\nBattery Level : ${batteryLevel}%\nIs Charging : ${charged}\n\n_Jam :_ ${moment(t * 1000).format('HH:mm:ss')}`, id)
                break 
		...
		case '@6285155078806':
		tobz.reply(from, `Kalau mau lihat command boy ketik *${prefix}help* ya`)
		break
		...
		if(isReg(obj)) return
         if(cekumur(cekage)) return
            tobz.sendTextWithMentions(from, 
`Hai kak *@${sender.id.replace('@c.us','')}*

*Kamu bisa lihat menu dibawah ini ya.*
----------------------
...
// By Gimenz
        ...
        default:
			...
			if (command.startsWith('bot')) {
				if(isReg(obj)) return
				if(cekumur(cekage)) return
                tobz.sendTextWithMentions(from, `Hai *@${sender.id.replace('@c.us','')}* ada yang bisa dibantu? ketik *${prefix}help* untuk melihat command`, id)
           }
