...
//
        ...
        case prefix+'nulismerah':
            ...
            argz = body.trim().split(' ')
            var slicedArgs = Array.prototype.slice.call(argz, 1);
            const diTulis3 = await slicedArgs.join(' ')
              ...
              /*  case prefix+'play':
                    ...
                    const mulaikah = yt.all[0].url
                        ...
                        .then((res) => {
                            ...
                            axios.get(`https://tinyurl.com/api-create.php?url=${dl_link}`)
                            .then(async (a) => {
                            if (Number(filesize) >= 15000) return juwen.sendFileFromUrl(from, thumb, `thumb.jpg`, `*Data Berhasil Didapatkan!*\n\n*Title* : ${title}\n*Ext* : MP3\n*Filesize* : ${filesizeF}\n*Link* : ${a.data}\n\n_Untuk durasi lebih dari batas disajikan dalam bentuk link_`, id)
                            const captions = `*「Data Berhasil Didapatkan 」*\n\n*Title* : ${title}\n*Ext* : MP3\n*Size* : ${filesizeF}\n\n_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
                            juwen.sendFileFromUrl(from, thumb, `thumb.jpg`, captions, id)
...
⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻⁻`)
       ...
       /* case prefix+'setpackname':
            ...
            argz = body.trim().split(' ')
            var slicedArgs = Array.prototype.slice.call(argz, 1);
            const packnameweem = await slicedArgs.join(' ')
            if (!packnameweem.includes('|')) return await juwen.reply(from, `Kirim perintah *${prefix}exif nama|author*`, id)
            const packnamewm2 = packnameweem.split('|')[0]
            const autorwm2 = packnameweem.split('|')[1]
            setting.autorwm = `${autorwm2}`
            setting.packnamewm = `${packnamewm2}`
            packnamewm = `${packnamewm2}`
            autorwm = `${autorwm2}`
            fs.writeFileSync('./lib/database/setting.json', JSON.stringify(setting))
            juwen.reply(from, `Berhasil Mengganti Stiker WM`, id)
            break
