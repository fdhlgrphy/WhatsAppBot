...
/* const apipanas = (imageUrl) => new Promise((resolve, reject) => {
    fetch(``, {
        method: 'GET',
    })
    .then(async res => {
...
const comics = (imageUrl) => new Promise((resolve, reject) => {
    fetch(`https://naufalhoster.xyz/tools/photo_comic?apikey=${naufalkey}&url=${encodeURIComponent(imageUrl)}`, {
        method: 'GET',
    })
    .then(async res => {
        const text = await res.json()
...
const nightvis1 = (imageUrl) => new Promise((resolve, reject) => {
    fetch(`https://naufalhoster.xyz/tools/night_vision?apikey=${naufalkey}&url=${encodeURIComponent(imageUrl)}`, {
        method: 'GET',
    })
    .then(async res => {
        const text = await res.json()
...
//exports.apipanas = apipanas;
exports.nightvis1 = nightvis1;