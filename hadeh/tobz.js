...
↬	${texxt}  `
     ...
     /*   case prefix+'antisticker':
        ...
        case prefix+'quotesnime':
            ...
            await limitAdd(serial)
            break
            case prefix+'fakeidentity':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            const fake = await axios.get(`https://docs-jojo.herokuapp.com/api/fake_identity`)
            fake_ = fake.data
            tobz.reply(from, `*PERHATIAN INI HANYA UNTUK HIBURAN SEMATA*\n*-------------------------------------*\n\n*Nama*: ${fake_.name}\n*Gender*: ${fake_.gender}\n*Umur*: ${fake_.age}\n*Birthday*: ${fake_.birtday}\n*Occupation*: ${fake_.occupation}\n*Address*: ${fake_.address}\n*Kode Pos*: ${fake_.zip_code}\n*State*: ${fake_.state}\n*Country*: ${fake_.country}\n*Email*: ${fake_.email}\n*Phone*: ${fake_.phone}\n*Card*: ${fake_.card}\n*Code*: ${fake_.code}\n*Date*: ${fake_.date}\n*Pin Code*: ${fake_.pin_code}\n*Weight*: ${fake_.weight}\n*Height*: ${fake_.height}\n*Blood Type*: ${fake_.blood_type}\n*Status*: ${fake_.status}`, id)
            await limitAdd(serial)
            break
        case prefix+'meme':
            if(isReg(obj)) return
...
➸ *Total Rekaman:* ${recording}`
          ...
          case prefix+'$':
            ...
            //console.log(sigot.result.response)
            break
            case prefix+'fml':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
